                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    <td class="px-6 py-4">{{ $order->paid_at->format('d M Y, H:i') }}</td>
                                    <td class="px-6 py-4 text-center">
                                        <button @click="modalOpen = true; selectedOrder = {{ $order->toJson() }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                            Lihat Invoice
                                        </button>
                                    </td>
                                </tr>
            ...
            <div x-show="modalOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="bg-white rounded-lg shadow-xl w-full max-w-2xl p-8 invoice-content" @click.stop>
                <div class="flex justify-between items-center pb-6 border-b">
                    <div>
                        ...
                        <p class="text-sm text-gray-500" x-text="selectedOrder ? '#' + selectedOrder.id : ''"></p>
                    </div>
                    <div class="text-right">
                        <h2 class="text-xl font-bold">Pusat Layanan Kemasan Sintesa</h2>
                        <p class="text-xs text-gray-500">M63M+652, Jl. Soekarno-Hatta, Kaduagung Tengah,</p>
                        <p class="text-xs text-gray-500">Kec. Cibadak, Kabupaten Lebak</p>
                        <p class="text-xs text-gray-500">Banten 42317</p>
                    </div>
                </div>
                <div class="flex justify-between mt-6" x-if="selectedOrder">
                    <div>
                        <p class="font-bold text-gray-700">Ditagihkan Kepada:</p>
                        <p class="text-gray-600" x-text="selectedOrder.customer_name"></p>
                    </div>
                    <div class="text-right">
                        <p class="font-bold text-gray-700">Tanggal Lunas:</p>
                        <p class="text-gray-600" x-text="new Date(selectedOrder.paid_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })"></p>
                    </div>
                </div>
                <div class="mt-8" x-if="selectedOrder">
                    <table class="w-full text-left">
                       <thead class="bg-slate-100"><tr><th class="p-3 text-sm font-semibold text-gray-600">Deskripsi</th><th class="p-3 text-sm font-semibold text-gray-600 text-center">Jumlah</th><th class="p-3 text-sm font-semibold text-gray-600 text-right">Subtotal</th></tr></thead>
                       <tbody><tr class="border-b"><td class="p-3"><p class="font-medium" x-text="selectedOrder.product_name"></p><p class="text-xs text-gray-500" x-text="selectedOrder.packaging_label + ' - ' + selectedOrder.size + ' / ' + selectedOrder.net_weight"></p></td><td class="p-3 text-center" x-text="selectedOrder.quantity + ' pcs'"></td><td class="p-3 text-right" x-text="'Rp ' + (selectedOrder.price_per_piece * selectedOrder.quantity).toLocaleString('id-ID')"></td></tr></tbody>
                    </table>
                </div>
                <div class="flex justify-end mt-6" x-if="selectedOrder">
                    <div class="w-1/3">
                ...
                <div class="mt-10 flex justify-between items-center no-print">
                    <div class="px-4 py-2 bg-green-100 text-green-700 text-sm font-bold rounded-md">
